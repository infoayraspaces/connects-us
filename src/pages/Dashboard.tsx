...
async function fetchKotiMes(mes: string, sheetId: string): Promise<{ingreso:number,egreso:number,utilidad:number,ocupacion:number}> {
  try {
...
function InformeKoti() {
  ...
  useEffect(() => {
    ...
    Promise.all(meses.map(m => fetchKotiMes(m, KOTI_SHEET_ID))).then(results => {
      setDatos(results);
      setLoading(false);
    });
  }, [anio]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex gap-6">
              <div className="text-center">
                <p className="text-2xl font-bold text-[#52b788]">{fmt(totalIngreso)}</p>
                <p className="text-xs text-white/40 uppercase tracking-wide">Ingresos {anio}</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-[#e9c46a]">{fmt(totalUtilidad)}</p>
                <p className="text-xs text-white/40 uppercase tracking-wide">Utilidad {anio}</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-white">{ocupPromedio.toFixed(1)}%</p>
                <p className="text-xs text-white/40 uppercase tracking-wide">Ocupación prom.</p>
      ...
      {/* Ocupación */}
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <h3 className="font-semibold text-gray-800 mb-6">Ocupación mensual — {anio}</h3>
        <div className="flex items-end gap-3 h-32">
          {datos.map((d,i) => {
            const pct = Math.min(d.ocupacion / 100, 1.1);
            const color = d.ocupacion >= 100 ? '#2d6a4f' : d.ocupacion >= 90 ? '#52b788' : d.ocupacion >= 80 ? '#c0843a' : '#e76f51';
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-xs font-medium text-gray-700">{d.ocupacion > 0 ? d.ocupacion.toFixed(1)+'%' : '—'}</span>
                <div className="w-full rounded-t-md" style={{ height: `${pct*90}%`, background: color }}></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        {/* Informe Koti Coliving */}
        {filtroProyecto === "Koti Coliving" && (
          <InformeKoti />
        )}
